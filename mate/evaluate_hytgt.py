...
COLUMNS = [
    ...
    Column(name='Mean Intentional Coverage Rate', fmt=lambda x: f'{100.0 * x:.3f}%',
           width=25, color='crimson', bold=True),
...
def evaluate(
    ...
    while env.episode_step < env.max_episode_steps:
        ...
        intentional_coverage_rates.append(env.intentional_coverage_rate)
        ...
        values = [
            ...
            np.mean(intentional_coverage_rates),
...
def main():  # pylint: disable=missing-function-docstring,too-many-branches,too-many-statements
    ...
    target_agents = []
    arbitrary_candidates = list(mate.ArbitraryTargetAgent().spawn(np.int64(env.num_intentional)))
    random_candidates = list(mate.RandomTargetAgent().spawn(np.int64(env.num_targets-env.num_intentional)))
    count_intentional = 0
    ...
    keys = [
        ...
        'Mean Transport Rate',
        'Mean Coverage Rate',
        'Mean Intentional Coverage Rate',
    ...
    if not args.no_render:
        ...
        try:
            ...
            postfix = OrderedDict([
                ...
                ('MeanIntentionalCoverageRate', f'{100.0 * np.mean(statuses["Mean Intentional Coverage Rate"]):.1f}%'),
